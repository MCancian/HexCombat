...
| `scripts/BOOTSCalculator.gd` | Instance-wrapper compat shim — forwards to `CombatCalculator.resolve_map_attack()`. |
| `scripts/GameState.gd` | Turn orchestrator — `resolve_turn()` calls movement → contested-hex discovery → `_resolve_combat_at()` per hex → FEBA retreats. |

## 3. Combat formula (transcribed from `CombatCalculator.resolve_map_attack`, line 23)
...
**Strengths** (`_sum_unit_strength`, line 173):
  `maneuver = Σ(strength_for_type(unit.type) × supply_effectiveness)` for each maneuver unit.

**Support strength** (`_support_strength`, line 183):
  `support = Σ(count[type] × SUPPORT_MULTIPLIERS[type])`
  Multipliers (line 14): `artillery:0.8`, `rocket_artillery:1.2`, `cas:1.4`, `crbm:0.6`, `rotary_wing:1.3`.

**Final strengths** (line 41–44):
  ```
  ...
  ```
  Ratios: `unmodified_ratio = attacker_unmodified / defender_unmodified` (line 53), `ratio = attacker_strength / defender_strength` (line 54). Defender-unmodified also floored to 0.1 for the ratio.

**Loss rolls** (line 56–58, order: attacker-roll → defender-roll → feba-roll):
  ```
...
**Loss counts** (line 69–70): `losses = round(unit_count × loss_rate)`.

**Min-one-loss rule** (line 72–77): if both sides present and both loss-counts are 0, the weaker side gets 1 loss (ratio ≥ 1 → defender loses 1, else attacker loses 1).

**Casualty selection** (`_select_casualties`, line 201): filter out units tagged `"artillery"`, then `dice.choose_indices(eligible.size(), min(loss_count, eligible.size()))` without replacement. Non-artillery only.
...
**Result label** (line 87–91): `ratio ≥ 1.2 → "Attacker Advantage"`, `ratio ≤ 0.85 → "Defender Advantage"`, else `"Contested"`.

## 4. Casualty selection

`_select_casualties` at `CombatCalculator.gd:201`:
...
## 5. Unit strength table (from `UnitStats.TYPE_DEFS`, line 6)
...
Fallback categories (`FALLBACK_CATEGORY_DEFS`, line 26) provide strength/tag values for unknown types via substring matching; a warning is emitted.

## 6. Force aggregation (`CombatForces`)

`maneuver_units(brigades)` — `CombatForces.gd:9`: iterates each brigade's `composition`, skips battalions where `is_support_type()` returns true (tag `"artillery"` or `"rotary_wing"`), and emits one `{brigade_id, type, supply_effectiveness:1.0}` dict per `battalion.qty`.

`support_counts(brigades)` — `CombatForces.gd:25`: sums `battalion.qty` by support type, routing via tags: `"rocket"` → `rocket_artillery`, `"artillery"` → `artillery`, `"rotary_wing"` → `rotary_wing`. Does not count `cas` or `crbm` — those are external munition strikes, not organic battalions.

In `GameState._resolve_combat_at()` (line 1053): forces are gathered per hex via `_combat_contributors_for()` which collects:
...
Red is always assigned as attacker, Green as defender.

## 7. Movement

Modes (`Movement.gd:9–10`): `"tactical"` and `"administrative"`.

Allowance (`move_allowance()`, line 35):
| Mode | Slow (leg) | Fast (mechanized/armor/tank) |
|---|---|---|
| Tactical | 1 hex | 2 hexes |
| Administrative | 10 hexes | 20 hexes |

Fast mobility (`is_fast_mobility()`, line 18): brigade's `nato_type` string contains `"mechanized"`, `"armor"`, or `"tank"` (case-insensitive). Composition is **ignored** — a deliberate divergence from TIV (see note at `Movement.gd:14`).

Administrative-moved brigades cannot commit to combat that turn (`GameState.gd:219`).

## 8. Turn flow (`GameState.resolve_turn`, line 162)

```
  1. resolve_ijfs_turn()          — Red joint fires (pre-landing)
  2. resolve_antiship_turn()       — Green anti-ship + mines
  3. resolve_offload_turn()        — Amphibious landing
  4. _apply_move_orders(RED)       — Planned movement
  5. _apply_move_orders(GREEN)
  6. _find_contested_hexes()       — Hexes with both teams present
  7. _resolve_combat_at(hex)       — Per contested hex (CombatCalculator)
  8. _apply_feba_retreats()        — Push defenders back by feba_km
  9. GameData.recompute_hex_ownership()
 10. resolve_supply_turn()
 11. resolve_cleanup_phase()
```

Combat sits at step 7, after all movement and before FEBA retreats/ownership changes.
