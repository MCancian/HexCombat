...
**D2 Red DOS Supply System (`SupplyResolver`, `SupplyState`, `TurnClosure`)**:
...
- **Resolver & Phase Integration**: `TurnClosure.resolve_supply_turn` calculates daily consumption and delegates to `SupplyResolver.resolve()`. The supply effectiveness multiplier is injected into combat resolution via `CombatResolver.inject_supply_effectiveness`, threaded by `TurnConductor`.
