...
4. **Anti-ship** — `FiresPhases.resolve_antiship_turn(dice)` — Green coastal anti-ship fires + mine warfare against the Red crossing. Applies IJFS writeback (destroyed/suppressed launchers), resolves firing plan, crossing damage, mine losses; drowned BNs leave their rosters via `ForceTransitions.apply_crossing_loss`, one preflighted all-or-nothing transaction. Sub-stream: `dice.derive("antiship:<turn>")`.
...
11. **Ground combat** — for each contested hex, `resolve_combat_at(hex_id, dice.derive("combat:<turn>:<hex>"))` runs `CombatResolver.resolve_at()`, applies casualties through `ForceTransitions.apply_battalion_casualties`, accumulates FEBA movement.
...
14. **Supply** — `TurnClosure.resolve_supply_turn()`: `SupplyBill.for_turn` bills every LANDED Red battalion and `SupplyTransitions.apply_daily_bill` decrements `supply_state.current_dos_tons`. Runs after combat so the bill covers who actually fought.
...
16. **End-of-turn tripwires** (debug builds only) — `GameData.validate_runtime_indexes()` and `ForceTransitions.pending_pool_roster_violations()` assert at the settled boundary.
