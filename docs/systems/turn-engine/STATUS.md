...
- **On-demand resolution maps**: `python3 tools/generate_resolution_dag.py --validate` reflects the current GDScript symbol surface, conservatively traces call-site reads/writes/RNG use, and regenerates designer-facing pages under `docs/presentations/`. The pages show uncertainty and source hashes; they are reading aids, not a scheduler or freshness gate.
