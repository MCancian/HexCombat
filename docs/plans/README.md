...
| 0061 | [Model a resolution day as an explicit DAG, substream per node](0061-resolution-dag.md) | Medium (**SPLIT by USER ruling 2026-08-01**. Part 1 — an ON-DEMAND artifact builder, a skill in the `hexcombat-structure-map` mould that regenerates the resolution-graph page from the tree, allowed to go stale, agy-buildable; the 18-edge read/write inventory is its input. Needs no golden re-baseline — do this first. Part 2 — substream-per-node (`Dice.derive` per step) is now a SEPARATE future plan and must be re-argued on determinism/legibility grounds: its stated payoff was 'ordering questions get cheap', and the ordering question was measured down to 15.6 airframes of 584 confined to turns 2-6) | Part 1 ready to plan; Part 2 deferred, unargued |
| 0060 | [Should air attrition happen before Red strikes?](0060-air-attrition-before-the-strike.md) | Medium-Large (balance + mechanic. **USER design session held 2026-08-01 — nine rulings recorded IN the plan.** Its headline question is ANSWERED BY REPLACEMENT: the two MANPADS mechanics collapse into ONE (interception, which already runs before each strike, gains abort AND attrition as one roll with three outcomes; the island-wide contest is deleted), across all roles, differentiated by role-exposure multipliers. **R9 re-baselines the force: J-16D 48 -> 10, so Red's OOB is 546, not 584** — every earlier number in the plan is historical, and the load-bearing anchors are restated at the new force (58.1 air losses/game, 10.6%; post-strike share falls 43.6% -> 28.4%). Also ruled: standoff warmup, `role_exposure_multipliers` = altitude exposure, turn-2 SAM annihilation is the base case, OWA drones are an expendable pool outside the OOB. **Re-scope and retitle when work starts** — this is no longer a reordering plan. Golden re-baseline + study re-run certain; plan alongside 0059 step 2, which it collides with) | **Sketch — DESIGN COMPLETE 2026-08-01, twelve rulings, nothing blocked on the USER.** Land R9 (J-16D 10) + R11 (rename HARM) FIRST — every downstream calibration is fitted against the force they define |
| 0059 | [Readable air inventory, then SAM interception and return-to-base](0059-sam-interception-and-rtb.md) | Medium (USER calls 2026-08-01. **Step 1 is additive and independently useful**: Red's fixed 584-airframe inventory already ticks down campaign-long, but the per-squadron OOB is unreadable — `FiresPhases` drops `air_oob_after` and only `red_air_losses` is surfaced. **Step 2 is the mechanic**: `rtb_today` is never written and the SAM side that would drive it does not exist — all three air-attrition paths are binary kill/no-kill, so a SAM target has three outcomes and an aircraft has two. Golden re-baseline expected for step 2 only, since any new draw shifts the stream) | Sketch |
...
| 0056 | [Make the coupling budget opt-out](../archive/0056-coupling-budget-opt-out.md) | Medium (risk buydown; dependency ceilings policed 5 files of 167 because they were opt-in, while the parameter cap was opt-out and universal. Enforces only — fixes no coupling) | **COMPLETE** (2026-08-01) |
