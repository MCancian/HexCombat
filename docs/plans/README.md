...
| 0061 | [Model a resolution day as an explicit DAG, substream per node](0061-resolution-dag.md) | Large (architecture; USER direction 2026-08-01. The six steps of an IJFS day share ONE dice stream, which is why any ordering change costs a full golden re-baseline and entangles RNG movement with balance movement. A derived substream per node — the pattern already used for `combat:<turn>:<hex>` and `ijfs:<turn>:<day>` — makes reordering cost only the balance delta it causes. One-time re-baseline; recommends declared-and-validated over an execution engine) | **Sketch — BLOCKED on the same USER design session as 0060**; scope of "all the calculators" unconfirmed |
| 0060 | [Should air attrition happen before Red strikes?](0060-air-attrition-before-the-strike.md) | Medium (balance; USER question 2026-08-01. Two of three air-attrition sources fire AFTER the strike, so aircraft killed by MANPADS or the free shot still flew their mission. Moving them is NOT a neutral reorder — killed aircraft stop striking AND more shooters are alive to kill them, compounding into a real Red nerf; the free shot's post-strike position is definitional. Certain golden re-baseline + study re-run; the sensitivity comparison IS the deliverable) | **Sketch — BLOCKED on a USER design session** (their call 2026-08-01); do not implement or pick an option |
