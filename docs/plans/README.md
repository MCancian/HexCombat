...
| 0059 | [SAM interception and return-to-base](0059-sam-interception-and-rtb.md) | Medium (new mechanic; USER call 2026-08-01 — `rtb_today` is a field the pipeline never writes, and the SAM side that would drive it does not exist: all three air-attrition paths are binary kill/no-kill, so a SAM target has three outcomes and an aircraft has two. Four design calls open; golden re-baseline expected since any new draw shifts the stream) | Sketch |
