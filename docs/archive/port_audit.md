...
| **Ground-casualty IJFS↔OOB linkage** | ✅ DONE 2026-06-29 (overnight 2b–2d) | **CLOSED.** `IjfsLoaders.build_maneuver_targets` generates per-battalion "Maneuver Units" IJFS targets from the ROC OOB (`{brigade_id}-MU-{n}`); wired into `_rebuild_ijfs_state` so they're struck; `_compute_ijfs_writeback` populates `maneuver_casualties`; `GameState._apply_ijfs_maneuver_casualties` removes the struck battalions from the OOB before ground combat. Tests: `ijfs_maneuver_targets_test.gd`, `ijfs_maneuver_consume_test.gd`. Golden unchanged. Detection/lethality bias (2c-ii) DONE 2026-06-30: mobility/hardness via `MANEUVER_TYPE_MAP` profiles; posture-by-activity via `GameState._update_maneuver_posture` (recently-active brigades → `posture="active"` → more detectable). `ijfs_maneuver_posture_test.gd`. Per-turn OOB sync (2d follow-up) DONE 2026-06-30: `GameState._sync_maneuver_targets_to_oob` retires maneuver targets whose battalions died (IJFS or ground combat) — sync not rebuild, so survivor detection continuity is preserved. `ijfs_maneuver_sync_test.gd`. _Original notes:_ `maneuver_casualties` was *computed* (`GameState._compute_ijfs_writeback`, a port of `ijfs_maneuver_writeback_service`) and exposed via the LLM observation — but it is **not consumed**: nothing removes the struck battalions from the ground OOB, so IJFS air/missile kills don't reduce the brigades that fight. The remaining half is (a) the ID bridge so IJFS maneuver-target `battalion_id`/`brigade_id` match the PLA/ROC OOB, and (b) applying the casualties before ground combat. By contrast the anti-ship half of the writeback IS consumed (feeds the D3 firing plan). Design settled 2026-06-28 (Option B + detectability); needs `moved_last_turn`/`fought_last_turn` on `Brigade`. |
...
| **Per-ship-type mine neutralization likelihood** | ✅ DONE 2026-06-30 | Optional `ShipDef.mine_neutralization_likelihood` (from `ships.json`) overrides the per-category table in `GameState._mine_ship_meta` (decoy override > per-hull > category fallback). Additive/byte-stable — no hull overrides yet; the field is the per-hull tuning hook. `mine_neutralization_override_test.gd`. |
