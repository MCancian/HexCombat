...
var _h := ValidatorHarness.new("Beach data validation")
var _hex_ids: Array = []
...
func _validate_count(beaches_data: Array) -> void:
	...
	if count != EXPECTED_BEACH_COUNT:
		_h.fail("Beach count changed: expected %d, got %d" % [EXPECTED_BEACH_COUNT, count])
	else:
...
func _validate_beach_contracts(beaches_data: Array) -> void:
	...
	for beach_data in beaches_data:
		...
		if beach_id == 0:
			_h.fail("Beach missing id field")
			continue
		if beach_id in ids_seen:
			_h.fail("Duplicate beach id: %d" % beach_id)
		ids_seen.append(beach_id)

		var name_en := String(beach_data.get("name_en", ""))
		if name_en.is_empty():
			_h.fail("Beach %d missing name_en" % beach_id)

		var hex_id := String(beach_data.get("hex_id", ""))
		if hex_id.is_empty():
			_h.fail("Beach %d missing hex_id" % beach_id)
		elif hex_id not in _hex_ids:
			_h.fail("Beach %d hex_id '%s' not present in taiwan_hex_grid.json" % [beach_id, hex_id])

		var to_number := int(beach_data.get("to_number", -1))
		if to_number not in KNOWN_TO_NUMBERS:
			_h.fail("Beach %d has unknown to_number: %d" % [beach_id, to_number])

		var offload_rate := float(beach_data.get("offload_rate", 0.0))
		if offload_rate <= 0.0:
			_h.fail("Beach %d has non-positive offload_rate: %s" % [beach_id, offload_rate])

		var depth := int(beach_data.get("depth", 0))
		if depth <= 0:
			_h.fail("Beach %d has non-positive depth: %d" % [beach_id, depth])

		var capacity_bns := int(beach_data.get("capacity_battalions", 0))
		if capacity_bns <= 0:
			_h.fail("Beach %d has non-positive capacity_battalions: %d" % [beach_id, capacity_bns])

		var lat := float(beach_data.get("lat", 0.0))
		var lng := float(beach_data.get("lng", 0.0))
		if lat == 0.0 or lng == 0.0:
			_h.fail("Beach %d missing lat/lng (got lat=%s lng=%s)" % [beach_id, lat, lng])
