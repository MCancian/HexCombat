...
var _h := ValidatorHarness.new("Offload weights validation")
...
func _validate_structure() -> void:
	...
	if file == null:
		_h.fail("Could not open %s" % OFFLOAD_WEIGHTS_PATH)
		return

	var parsed: Variant = JSON.parse_string(file.get_as_text())
	if not (parsed is Dictionary):
		_h.fail("offload_weights.json did not parse to a Dictionary")
		return
	...
	if not (default_tons is float or default_tons is int) or float(default_tons) <= 0.0:
		_h.fail("default_tons must be > 0, got %s" % default_tons)

	var weights: Dictionary = parsed.get("weights", {})
	if weights.is_empty():
		_h.fail("missing or empty 'weights' Dictionary")
		return

	for key in weights.keys():
		if not UnitStats.TYPE_DEFS.has(key):
			_h.fail("weights key '%s' is not a known UnitStats type" % key)

	for key in UnitStats.TYPE_DEFS.keys():
		if not weights.has(key):
			_h.fail("TYPE_DEFS key '%s' missing from weights" % key)

	for key in weights.keys():
		var val: Variant = weights[key]
		if not (val is float or val is int) or float(val) <= 0.0:
			_h.fail("weight for '%s' must be > 0, got %s" % [key, val])

	var bn_class_of: Dictionary = parsed.get("bn_class_of", {})
	for key in bn_class_of.keys():
		if not UnitStats.TYPE_DEFS.has(key):
			_h.fail("bn_class_of key '%s' is not a known UnitStats type" % key)
	...
	for key in multipliers.keys():
		if not allowed_kinds.has(key):
			_h.fail("multipliers top-level key '%s' not in {beach, port, airbridge, default}" % key)
...
func _validate_multiplier_leaves(d: Dictionary, path: String) -> void:
	for key in d.keys():
		...
		if val is Dictionary:
			...
			if float(val) <= 0.0:
				_h.fail("multiplier leaf at '%s' must be > 0, got %s" % [child_path, val])
		else:
			_h.fail("multiplier at '%s' is neither Dictionary nor number: %s" % [child_path, val])


func _validate_ship_categories() -> void:
	var file := FileAccess.open(SHIPS_PATH, FileAccess.READ)
	if file == null:
		_h.fail("Could not open %s" % SHIPS_PATH)
		return

	var parsed: Variant = JSON.parse_string(file.get_as_text())
	if not (parsed is Dictionary):
		_h.fail("ships.json did not parse to a Dictionary")
		return
	...
	for cat in found.keys():
		if cat != "default" and not known_categories.has(cat):
			_h.fail("ship_category '%s' in multipliers not found in ships.json entries" % cat)
...
func _validate_spot_checks() -> void:
	...
	if not is_equal_approx(check1, 1100.0):
		_h.fail("spot-check 1: expected 1100.0, got %s" % check1)
	...
	if not is_equal_approx(check2, 4400.0):
		_h.fail("spot-check 2: expected 4400.0, got %s" % check2)
	...
	if not is_equal_approx(check3, 2200.0):
		_h.fail("spot-check 3: expected 2200.0, got %s" % check3)
	...
	if not is_equal_approx(check4, OffloadRates.TONS_PER_BN):
		_h.fail("spot-check 4: expected %s, got %s" % [OffloadRates.TONS_PER_BN, check4])
