...
var _h := ValidatorHarness.new("combat data validation")
...
func _initialize() -> void:
	...
	if data.is_empty():
		_h.fail("Unable to load %s" % OOB_PATH)
		_h.finish(self)
		return
...
func _validate_unit_types(data: Dictionary) -> void:
	...
	for unit_type in unit_types:
		...
		if category.is_empty():
			_h.fail("No category resolved for battalion type: %s" % unit_type)
...
func _validate_support_normalization() -> void:
	...
	_h.equal_int("support artillery preserves nonzero count", support["artillery"], 5)
	_h.equal_int("support explicit null becomes zero", support["rocket_artillery"], 0)
	_h.equal_int("support missing key becomes zero", support["crbm"], 0)
	_h.equal_int("support negative clamps to zero", support["cas"], 0)
...
func _validate_regression_samples(data: Dictionary) -> void:
	...
	for brigade_id in EXPECTED_BRIGADE_STRENGTHS:
		...
		if brigade.is_empty():
			_h.fail("Missing regression brigade: %s" % brigade_id)
			continue
		...
		if not is_equal_approx(strength, expected):
			_h.fail("Brigade %s strength changed: expected %.1f, got %.1f" % [brigade_id, expected, strength])
...
func _validate_artillery_classification(data: Dictionary) -> void:
	...
	if actual != expected:
		_h.fail("Artillery classification changed: expected %s, got %s" % [", ".join(expected), ", ".join(actual)])
