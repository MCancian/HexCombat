...
var _h := ValidatorHarness.new("headless IJFS validation")
...
func _validate_state_loaded() -> void:
	...
	_h.is_true("ijfs_state is null before first turn (lazy)", GameState.ijfs_state == null)
	_h.equal_int("ijfs day starts at 0", GameState._ijfs_day, 0)
	_h.is_true("last_ijfs_summary starts empty", GameState.last_ijfs_summary.is_empty())
...
func _validate_day1_run() -> void:
	...
	_h.equal_int("ijfs day advanced to 1", GameState._ijfs_day, 1)
	_h.is_true("ijfs_state lazy-loaded on first turn", GameState.ijfs_state != null)
	_h.is_true("ijfs_state has targets", (GameState.ijfs_state.targets as Array).size() > 0)
	_h.is_true("ijfs_state has squadrons", GameState.ijfs_state.squadron_force != null and (GameState.ijfs_state.squadron_force as Array).size() > 0)

	for key in ["detection_log", "strike_log", "target_status_after", "munition_inventory_after", "engagement_log", "summary", "air_oob_after"]:
		_h.is_true("ledger has %s" % key, ledgers.has(key))
	_h.is_true("air_oob_after model_version 4", int((ledgers["air_oob_after"] as Dictionary).get("model_version", 0)) == 4)

	var summary: Dictionary = GameState.last_ijfs_summary
	_h.is_true("summary not empty", not summary.is_empty())
	for key in ["target_counts_by_category_status", "taiwan_ad_health_after", "attacks", "red_air_losses"]:
		_h.is_true("summary has %s" % key, summary.has(key))
	_h.is_true("attacks.executed is int >= 0", int((summary["attacks"] as Dictionary).get("executed", -1)) >= 0)

	var writeback: Dictionary = GameState.last_ijfs_writeback.to_dict()
	for key in WRITEBACK_KEYS:
		_h.is_true("writeback has %s" % key, writeback.has(key))
	_h.is_true("antiship_destroyed_by_type is a Dictionary", writeback["antiship_destroyed_by_type"] is Dictionary)
	_h.is_true("maneuver_casualties is an Array", writeback["maneuver_casualties"] is Array)
	_h.is_true("sam_destroyed >= 0", int(writeback["sam_destroyed"]) >= 0)
...
func _validate_determinism() -> void:
	...
	_h.is_true("same seed -> identical IJFS summary", first == second)
	_h.is_true("same seed -> identical IJFS writeback", first_wb == second_wb)
...
func _validate_continuity() -> void:
	...
	GameState.resolve_ijfs_turn(SeededDice.new(SEED))
	_h.equal_int("ijfs day advanced to 2", GameState._ijfs_day, 2)
	var destroyed_day2 := _destroyed_target_count()
	_h.is_true("destroyed targets persist across days (non-decreasing)", destroyed_day2 >= destroyed_day1)
	...
	for target in GameState.ijfs_state.targets:
		if target.suppressed_this_turn and not target.suppressed:
			stuck_suppressed += 1
	_h.equal_int("no stale suppressed_this_turn after carry", stuck_suppressed, 0)


func _validate_observation() -> void:
	var obs: Dictionary = LLMGameAPI.observation("Red")
	_h.is_true("observation has ijfs block", obs.has("ijfs"))
	...
	for key in ["resolved_day", "attacks", "taiwan_ad_health_after", "antiship_destroyed_by_type", "sam_destroyed", "maneuver_casualties"]:
		_h.is_true("ijfs observation has %s" % key, ijfs.has(key))
	_h.equal_int("ijfs observation resolved_day == 2", int(ijfs["resolved_day"]), 2)
