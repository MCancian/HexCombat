...
var _h := ValidatorHarness.new("Theater data validation")
...
func _parse_int_array(value: Variant) -> Array[int]:
	...
	if not (value is Array):
		_h.fail("Expected Array, got %s" % type_string(typeof(value)))
		return result
	for item in value:
...
func _parse_int_array_map(value: Variant, field_name: String) -> Dictionary:
	...
	if not (value is Dictionary):
		_h.fail("%s must be a Dictionary" % field_name)
		return result
	for key in value.keys():
		var map_value = value[key]
		if not (map_value is Array):
			_h.fail("%s[%s] must be an Array" % [field_name, String(key)])
			continue
...
func _assert_active_tos(active_tos: Array[int]) -> void:
	if active_tos != EXPECTED_ACTIVE_TOS:
		_h.fail("active_tos changed: expected %s, got %s" % [EXPECTED_ACTIVE_TOS, active_tos])
	else:
...
func _assert_adjacency(to_adjacency: Dictionary, active_tos: Array[int]) -> void:
	for to in to_adjacency.keys():
		...
		if to_number not in active_tos:
			_h.fail("Adjacency key is not an active TO: %d" % to_number)
		var neighbors: Array[int] = to_adjacency[to_number]
		for neighbor in neighbors:
			if neighbor not in active_tos:
				_h.fail("TO %d adjacency endpoint is not active: %d" % [to_number, neighbor])
			elif not (neighbor in to_adjacency):
				_h.fail("TO %d adjacency endpoint missing adjacency list: %d" % [to_number, neighbor])
			else:
				var reverse_neighbors: Array[int] = to_adjacency[neighbor]
				if to_number not in reverse_neighbors:
					_h.fail("TO adjacency is not symmetric: %d -> %d exists, but reverse is missing" % [to_number, neighbor])

	for active_to in active_tos:
		if active_to not in to_adjacency:
			_h.fail("Active TO missing adjacency list: %d" % active_to)
...
func _assert_beach_to_to(beach_to_to: Dictionary) -> void:
	if beach_to_to.size() != EXPECTED_BEACH_COUNT:
		_h.fail("beach_to_to count changed: expected %d, got %d" % [EXPECTED_BEACH_COUNT, beach_to_to.size()])
	for beach_id in range(1, EXPECTED_BEACH_COUNT + 1):
		if beach_id not in beach_to_to:
			_h.fail("beach_to_to missing beach id: %d" % beach_id)
...
func _assert_beaches_agree(beaches_data: Array, beach_to_to: Dictionary) -> void:
	if beaches_data.size() != EXPECTED_BEACH_COUNT:
		_h.fail("beaches.json count changed: expected %d, got %d" % [EXPECTED_BEACH_COUNT, beaches_data.size()])
	for beach_data in beaches_data:
		var beach_id := int(beach_data.get("id", 0))
		if beach_id == 0:
			_h.fail("beaches.json beach missing id")
			continue
		if beach_id not in beach_to_to:
			_h.fail("beaches.json beach %d missing from theater beach_to_to" % beach_id)
			continue
		...
		if beach_to_number != theater_to_number:
			_h.fail("Beach %d to_number mismatch: beaches.json=%d theaters.json=%d" % [beach_id, beach_to_number, theater_to_number])
