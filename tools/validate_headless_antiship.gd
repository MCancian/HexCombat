...
var _h := ValidatorHarness.new("headless anti-ship validation")
...
func _validate_run_and_reconcile() -> void:
	...
	_h.is_true("anti-ship phase produced a summary (reserve had a crossing wave)", not summary.is_empty())
	...
	_h.is_true("sent fleet is non-empty", not (summary["sent_by_type"] as Dictionary).is_empty())

	var lost := int(summary["bns_lost_at_sea"])
	_h.is_true("bns_lost_at_sea >= 0", lost >= 0)
	_h.equal_int("BNs removed from reserve == bns_lost_at_sea", bns_before - _reserve_bn_count(), lost)
	_h.equal_int("pending_lost_at_sea == bns_lost_at_sea (offload seam)", int(GameState.pending_lost_at_sea), lost)

	# Total ship hulls destroyed across crossing + mines.
	var hulls := 0
	for c in (summary["destroyed_by_ship_type"] as Dictionary).values():
		hulls += int(c)
	_h.is_true("ship hulls destroyed >= 0", hulls >= 0)
	...
	for state in GameState.fleet.values():
		if not state.validate():
			ok_fleet = false
	_h.is_true("all fleet ShipState invariants hold after losses", ok_fleet)
...
func _validate_determinism() -> void:
	...
	_h.is_true("same seed -> identical anti-ship summary", first == second)
...
func _validate_c2_suppression_reduces_firing() -> void:
	...
	if target_tos.is_empty():
		_h.fail("C2 suppression: no assaulted TO to probe")
		return
...
func _validate_cumulative_ijfs_attrition() -> void:
	...
	_h.is_true("multi-day IJFS attrited some anti-ship systems (writeback destroyed > 0)", wb_sum > 0)
	_h.equal_int("writeback destroyed == cumulative target-state destroyed", wb_sum, state_sum)
