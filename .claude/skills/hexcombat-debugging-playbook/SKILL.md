...
| "Fixture drift: LLMFixtures changed docs/examples/" | JSON contract drift | The gate already regenerated them via `tools/export_llm_*.gd`; read `git diff docs/examples/`. Intended growth → commit regen + schema + keys. Unintended → your change leaked into serialization. |
