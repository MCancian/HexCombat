...
|---|---|---|
| `data/taiwan_hex_grid.json` | 455 hexes, odd-r offset coords + centers | (loaded in smoke) |
...
(300/150/150); offload only in `OffloadRates.gd`. `CombatCalculator`'s `TERRAIN_MODIFIERS` (all
effectively 1.0 — terrain deferred) and `SUPPORT_MULTIPLIERS` are code constants today —
data-driven promotion is fine if a scenario variant needs them. Known wart: `CombatCalculator.gd`
carries a stale `feba_base_km=2.0` default param; real callers pass the scenario value.
