...
## carry an optional `mine_neutralization_likelihood`; AntishipResolver.mine_ship_meta prefers it over
## the per-category table (which remains the fallback). Decoys keep their own transit override. Pure
## lookup, no dice.
...
func test_override_beats_category_and_fallback_holds() -> void:
	...
	var ship_defs := {base.id: base, over.id: over, decoy.id: decoy}

	var transit := {
		"neutralization_likelihood_by_category": {"TESTCAT": "low"},
		"decoy_neutralization_likelihood": "high",
	}
	var meta: Dictionary = AntishipResolver.mine_ship_meta(ship_defs, transit)
...
func test_real_ships_default_to_category_unchanged() -> void:
	...
	var meta: Dictionary = AntishipResolver.mine_ship_meta(GameData.ship_defs, transit)
