## Verifies CleanupResolver.census counts PRESENT (landed) battalions, not the full OOB
...
func _census() -> Dictionary:
	return CleanupResolver.census(
		GameData.brigades, GameState.data.pending_battalion_pools(), GameData.victory_config)
...
func test_partially_landed_brigade_counts_only_landed_bns() -> void:
	...
	var baseline_red := int(_census()["red"])
...
func test_taiwan_hexes_filter_excludes_offshore_hex() -> void:
	...
	var baseline_green := int(_census()["green"])
	...
	assert_int(int(_census()["green"])).is_equal(baseline_green)
	...
	var after_green := int(_census()["green"])
