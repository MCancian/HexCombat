...
## Neutral exposure for every role, so the cases below still assert the RCS-only arithmetic they
## were written against; role exposure gets its own suite (ijfs_attrition_profile_test).
const NEUTRAL_ROLES := {"role_exposure_multipliers": {"isr": 1.0, "sead": 1.0, "strike": 1.0}}


func _profile(role_exposure: Dictionary = NEUTRAL_ROLES) -> IjfsAttritionProfile:
	return IjfsAttritionProfile.build({"red_aircraft_attrition_and_sead": role_exposure}, AIR_CLASSES)
...
func test_sead_destroys_target_when_roll_below_p_destroy() -> void:
	...
	var result := IjfsEngagement.resolve_sead_engagement(targets, force, _profile(), dice)
...
func test_sead_disabled_sets_unengaged_and_draws_nothing() -> void:
	...
	var result := IjfsEngagement.resolve_sead_engagement(targets, force, _profile(), dice, false)
...
func test_null_force_returns_empty_logs() -> void:
	...
	var dice := ScriptedDice.new([], [], [])
	var result := IjfsEngagement.resolve_sead_engagement(targets, null, _profile(), dice)
	assert_array(result["engagement_log"]).is_empty()
	assert_array(result["contest_log"]).is_empty()
...
func test_free_shot_skipped_when_no_sam_health() -> void:
	...
	var force: Array[IjfsSquadron] = [sq]
	var dice := ScriptedDice.new([], [], [])
	var log := IjfsEngagement.apply_post_phase_2_free_shot(force, _profile(), 0.0, dice)
